...
class CaptionProcessor(object):
    def __init__(self, copyright_rate, character_rate, general_rate, artist_rate, normalize, caption_shuffle,
                 transforms, max_size, resize, random_order):
    ...
    def clean(self, text: str):
        text = ' '.join(
            set([i.lstrip('_').rstrip('_') for i in re.sub(r'\([^)]*\)', '', text).split(' ')])).lstrip().rstrip()
...
class LocalBase(Dataset):
    def __init__(self,
        ...
        for i in self.image_files:
            ...
            self.examples[hash] = {
                'image': i,
                KEY_TEXT: f'{data_root}/texts/{hash}.txt'
            }
    ...
    def get_caption(self, i):
        ...
        caption = open(example[KEY_TEXT], 'r').read()
...
if __name__ == "__main__":
    dataset = LocalBase('./dataset', size=512, crop=False, mode='val')
